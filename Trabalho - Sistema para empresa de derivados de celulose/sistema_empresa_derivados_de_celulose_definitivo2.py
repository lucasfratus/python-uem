...
def repeticao_vendedor(lst: list[Vendedor_Candidato]) -> list[Vendedor_Candidato]:
    ...
    [Vendedor_Candidato(nome='Lucas', soma_quantidades=22000, lucro_por_vendedor=110000.0), Vendedor_Candidato(nome='Lucas', soma_quantidades=0, lucro_por_vendedor=0.0)]
        ...
        for cv in range(len(lst)):
            for v in range(cv + 1, len(lst)):
                if lst[cv].nome == lst[v].nome:
                    ...
                    lst[v].soma_quantidades = 0  # Zera a quantidade das próximas vezes que o mesmo vendedor aparece
    return lst
